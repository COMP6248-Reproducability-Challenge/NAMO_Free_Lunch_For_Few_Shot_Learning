...
# ========================================================
#   Usefull paths
_datasetFeaturesFiles = {"miniImagenet": "./filelists/miniImagenet/novel_features.plk", # /WideResNet28_10_S2M2_R/last
                         "CUB": "./filelists/CUB/novel_features.plk",} #/WideResNet28_10_S2M2_R/last
_cacheDir = "./cache"
_maxRuns = 10000
_min_examples = -1
...
def GenerateOrigin(cfg):
    global _randStates, data, _min_examples
    np.random.set_state(_randStates[0])
    classes = np.random.permutation(np.arange(data.shape[0]))[:cfg["ways"]]
    shuffle_indices = np.arange(_min_examples)
    dataset_origin = torch.zeros((cfg['ways'],data.shape[1],data.shape[2]))

    

    for i in range(cfg['ways']):
        shuffle_indices = np.random.permutation(shuffle_indices)
        dataset_origin[i] = data[classes[i], shuffle_indices,
                              :]
    return dataset_origin 
...
if __name__ == "__main__":
    ...
    loadDataSet('miniimagenet') #'miniimagenet''CUB'
